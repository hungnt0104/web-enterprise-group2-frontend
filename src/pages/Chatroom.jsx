...
const Chatroom = ({ socket }) => {
    ...
    return (
        <div className="container" style={{ height: '100vh', width: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center', backgroundImage: 'url(\'https://getwallpapers.com/wallpaper/full/e/b/f/1029750-vertical-black-elegant-wallpaper-1920x1080.jpg\')', backgroundSize: 'cover', backgroundPosition: 'center' }}>
            <div className="formContainer" style={{ width: '400px', margin: '0 auto 0 auto', padding: '32px', borderRadius: '6px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '28px', background: 'rgba(255, 255, 255, 0.5)' }}>
                <h1 style={{ color: 'white' }}>Message Room</h1>
                <div style={{ color: 'white' }}>Name: <strong>{name}</strong></div>
                <div style={{ color: 'white' }}>Email: <strong>{email}</strong></div>
                <div style={{ color: 'white' }}>Department: <strong>{department}</strong></div>
                <button className="btn btn-primary" style={{ backgroundColor: 'black', color: 'white' }} onClick={joinRoom}>Join Room</button>
                <button className="btn btn-secondary" style={{ backgroundColor: 'black', color: 'white' }} onClick={() => { window.location.href = '/home' }}>Cancel</button>
            </div>
        </div>
